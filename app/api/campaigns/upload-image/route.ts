...
/** Upload de imagem de anúncio pelo parceiro — mesma validação/processamento
 *  real de app/api/upload/admin-image/route.ts (mime real, redimensiona,
 *  webp). O bucket project-images só aceita insert de técnico via RLS, então
 *  o upload em si usa o client admin depois de validar a imagem de verdade
 *  no servidor (a associação com uma campanha específica só acontece na
 *  rota de creative, que checa dono). */
export async function POST(req: NextRequest) {
  try {
    ...
    const userLimit = await checkRateLimit({ key: `upload:campaign-image:${user.id}`, limit: 30, windowMs: 3_600_000 })
    if (!userLimit.allowed) return NextResponse.json({ error: 'Limite de upload atingido. Tente novamente mais tarde.' }, { status: 429 })
    const ipLimit = await checkRateLimit({ key: `upload:campaign-image:ip:${ip}`, limit: 100, windowMs: 3_600_000 })
    ...
    const { error: uploadError } = await admin.storage.from(BUCKET).upload(storagePath, processed.buffer, {
    ...
    console.error('[campaigns/upload-image partner]', err)
